...
int main(void)
{
	...
	FTM2_SetPeriod(0, 12500);		/* Interrupt every 12500 counts,  equal to every 100 ms */
...
uint32_t ADC_FTM_ValueFunction()
{
	...
	if (adc_value >=0 && adc_value < 85)
	{
		...
		{
			FLEX_value = 12500; /* Refresh interrupt period */
		}
		else if (adc_value >= 28 && adc_value < 56)
		{
			FLEX_value = 25000; /* Refresh interrupt period */
		}
		else if (adc_value >= 56  &&  adc_value < 85)
		{
			FLEX_value = 37500; /* Refresh interrupt period */
		}
