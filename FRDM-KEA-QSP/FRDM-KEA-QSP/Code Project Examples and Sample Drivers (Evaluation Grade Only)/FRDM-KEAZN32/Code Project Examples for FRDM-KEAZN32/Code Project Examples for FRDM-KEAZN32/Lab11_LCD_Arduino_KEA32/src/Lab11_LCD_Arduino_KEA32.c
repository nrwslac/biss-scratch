...
#elif defined(MCU_SKEAZN642)
	//volatile KBI_MemMapPtr pKBI = KBI0;
#else
...
#elif defined(MCU_SKEAZN642)
	uint32_t value = (GPIOA_PDIR & 0b11); //Grab the GPIO value to determine which key is pressed
#else
	/* Write the code for how your device determines what key is pressed. */
#endif
...
#elif defined(MCU_SKEAZN642)
	/* SKEAZN64 family has no KBIx_SC_RSTKBSP field nor KBIx_SP register. */
#else
#endif
