...
void Clk_Init()
{
	...
	SIM_BUSDIV |= 0x01; //core clock = ICSOUT/1 and bus clock = core clock/2
#endif
...
#elif defined(MCU_SKEAZN642)
	CONFIG_PIN_AS_GPIO(PORT_A, 0, INPUT);
	CONFIG_PIN_AS_GPIO(PORT_A, 1, INPUT);
	ENABLE_INPUT(PORT_A, 0);
	ENABLE_INPUT(PORT_A, 1);
#else
...
int main(void)
{
		...
		NVIC_EnableIRQ(UART1_IRQn);
#else
		/*Enable the UART interrupts for your MCU.*/
#endif
...
#elif defined(MCU_SKEAZN642)
	   	if((GPIOA_PDIR&GPIO_PDIR_PDI(0x00000001))>>0==1)
#else
	   		/* Read the correct GPIO for your device. */
#endif
	   	{
...
#elif defined(MCU_SKEAZN642)
	   	if((GPIOA_PDIR&GPIO_PDIR_PDI(0x00000002))>>1==1)
#else
	   		/* Read the correct GPIO for your device. */
#endif
	   	{
